...
const AutoplayPage = () => {
  ...
  const [currentIndex, setCurrentIndex] = useState(() => {
    return parseInt(localStorage.getItem('autoplayIndex') || '0');
  });
  ...
  useEffect(() => {
    localStorage.setItem('autoplayIndex', currentIndex.toString());
  }, [currentIndex]);
  ...
  if (videos.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">{t({ id: 'Tidak ada video tersedia', en: 'No videos available' })}</p>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 fade-in" data-testid="autoplay-page">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold" data-testid="autoplay-title">
          {t({ id: 'Mode Putar Otomatis', en: 'Autoplay Mode' })}
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="aspect-video rounded-lg overflow-hidden bg-black" data-testid="autoplay-player">
            <iframe
